...
FIJI plugin that reads .lif files from the Sp8 confocal microscope (including 3D stack images) and outputs a file system with .ome.tif files and a file system with png files. The png files can be directly used for setting up a memento project. Note however that in this plugin the image intensity scaling is done on the image level. If you prefer to scale intensities for all images of a well jointly, making images within a well more comparable, you should rather first export tif files with this plugin and then convert them to JPGs or PNGs by [another tool](https://github.com/CellProfiling/TifCs_To_HPA-PNG-JPEG) before uploading these files to Memento or using them for another further analysis purpose.
